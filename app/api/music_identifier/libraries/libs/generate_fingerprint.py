...
def generate_peaks(arr2d, min_amp=config.DEFAULT_AMP_MIN):
    ...
    neighborhood = iterate_structure(bin_struct, config.PEAK_NEIGHBORHOOD_SIZE)
    ...
    for i in range(len(peaks_list)):
        for j in range(1, fan_value):
            if (i + j) < len(peaks_list):
                ...
                if delta_time >= config.MIN_HASH_TIME_DELTA and delta_time <= config.MAX_HASH_TIME_DELTA:
                    h = hashlib.sha1()
