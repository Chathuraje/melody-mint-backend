...
def fingerprint(
    ...
    min_amp=config.DEFAULT_AMP_MIN,
    plots=False,
):
...
def generate_peaks(arr2d, min_amp=config.DEFAULT_AMP_MIN):
    """Finds local maxima.
    ...
    bin_struct = generate_binary_structure(2, 1)  # type: ignore
    neighborhood = iterate_structure(bin_struct, config.PEAK_NEIGHBORHOOD_SIZE)  # type: ignore

    local_max = maximum_filter(arr2d, footprint=neighborhood) == arr2d  # type: ignore

    background = arr2d == 0

    eroded_background = binary_erosion(
        background, structure=neighborhood, border_value=1  # type: ignore
    )
...
def generate_hashes(peaks, fan_value=config.DEFAULT_FAN_VALUE):
    ...
    for i in range(len(peaks_list)):
        for j in range(1, fan_value):
            if (i + j) < len(peaks_list):
                ...
                freq1 = peaks_list[i][config.IDX_FREQ_I]
                freq2 = peaks_list[i + j][config.IDX_FREQ_I]

                time1 = peaks_list[i][config.IDX_TIME_J]
                time2 = peaks_list[i + j][config.IDX_TIME_J]

                delta_time = time2 - time1

                if (
                    delta_time >= config.MIN_HASH_TIME_DELTA
                    and delta_time <= config.MAX_HASH_TIME_DELTA
                ):
                    h = hashlib.sha1()
                    h.update(
                        ("%s|%s|%s" % (str(freq1), str(freq2), str(delta_time))).encode(
                            "utf-8"
                        )
                    )
                    yield (h.hexdigest()[0 : config.FINGERPRINT_REDUCTION], time1)
