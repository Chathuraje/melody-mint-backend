...
def parse_bytes(bytes_data, format="mp3", offline=False):
    ...
    try:
        ...
        data = np.frombuffer(
            audio_file._data, np.int16
        )  # Fix: Replace np.fromstring with np.frombuffer
        channels = []
        ...
        logger.error("Error occurred while processing audio file: audiodro error")
        raise

    song = {"channels": channels, "frame_rate": frame_rate, "hash_value": hash_val}
