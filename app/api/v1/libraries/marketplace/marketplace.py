...
async def get_nfts(chain_id: int, collection_address: str) -> NFTResponse:
    ...
    for nft_id, owner_address in enumerate(result_nft):
        nft = SinglNFTResponse(nft_id=str(nft_id), owner_address=owner_address)
...
async def get_single_nft(
    ...
    result = contract.functions.getSingleNFT(nft_id).call()

    nft = SinglNFTResponse(nft_id=nft_id, owner_address=result[0])
